...
21. Current Progress
    ...
    - Upgrade modal popup on roadmap and fiqh tool interactions (instead of hard gate)
    Confirmed Business Logic (finalised in Step 15)
    - PDF downloads: FREE for all logged-in users — no subscription required
    - Roadmap: page visible to all logged-in users, but clicking a node → upgrade modal for free users
    - Fiqh Tool (Khilaf): page visible to all logged-in users, but search/category click → upgrade modal for free users
    - Unauthenticated users visiting roadmap or fiqh tool → redirected to #/login
    - GET /api/books (listing) is PUBLIC — no login required to browse books
    ...
    Key Fixes Made in Step 15
    - main.js getRoute() now strips query params before route matching (fixes #/account?payment=success → Page not found)
    - SecurityConfig: GET /api/books now permitAll (unauthenticated users can browse books)
    - Book "Usool al-Thalatha" (ID 1) reassigned from field "Aqeedah" (ID 1) to subfield "Ulohiyyah" (ID 6)
    - Account page, fiqh tool gate, upgrade modal: CSS added to style.css
    Notes for Next Session
    - hellohello@hotmail.com is the test account — currently 'paid'
    - masail Tests 5/6/7 deferred to Step 17 (need MasalahController)
    Up Next
    Step Checklist
    Step 16 — Masalah and MadhabOpinion models
...
architecture.md and progress.md are finished in the docs/ folder